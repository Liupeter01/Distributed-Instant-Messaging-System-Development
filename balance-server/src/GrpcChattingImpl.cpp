...
::grpc::Status grpc::GrpcChattingImpl::RegisterInstance(
    ...
    ::message::StatusResponse *response) {
          ...
          spdlog::info("[Balance Server]: Remote {0} Request To Register Chatting Instance Server",
                    request->info().name());
          ...
          if (details::GrpcDataLayer::get_instance()->findItemFromServer(request->info().name(),
                    ...
                    spdlog::warn("[Balance Server]: Remote {0} Try To Register Chatting Instance Failed"
          ...
          auto& lists = details::GrpcDataLayer::get_instance()->getChattingServerInstances();

          auto chatting_peer = std::make_unique<grpc::details::ServerInstanceConf>(
                              request->info().host(), request->info().port(),
                              request->info().name());

          lists.insert(std::make_pair(request->info().name(), std::move(chatting_peer)));

          spdlog::info("[Balance Server]: Remote  [{0}-{1}:{2}] Registered To Chatting Instances Server Successful!",
...
grpc::GrpcChattingImpl::RegisterGrpc(::grpc::ServerContext *context,
          ...
          spdlog::info("[Balance Server]: Remote {0} Request To Register Chatting GRPC Server",
                    request->info().name());

          //Already Exist
          if (details::GrpcDataLayer::get_instance()->findItemFromServer(request->info().name(),
                    details::SERVER_TYPE::CHATTING_GRPC_SERVER)) {

                    /*server has already exists*/
                    spdlog::warn("[Balance Server]: Remote {0} Try To Register Chatting GRPC Server Failed"
          ...
          spdlog::info("[Balance Server]: Remote  [{0}-{1}:{2}] Registered To Chatting GRPC Server Successful!",
                    request->info().name(), request->info().host(),
                    request->info().port());
...
::grpc::Status grpc::GrpcChattingImpl::ShutdownInstance(
    ...
    ::message::StatusResponse *response) {
          ...
          spdlog::info("[Balance Server]: Remote {0} Request To Remove Chatting Instance Server",
                    request->cur_server());
          ...
          if (status) {
                    spdlog::info("[Balance Server]: Remote [{0}] Removed From Chatting Instance Server Successful!",
                              request->cur_server());
          }
          else {
                    spdlog::warn("[Balance Server]: Remote [{0}] Removed From Chatting Instance Server Failed!",
                              request->cur_server());
          }
...
grpc::GrpcChattingImpl::ShutdownGrpc(::grpc::ServerContext *context,
          ...
          spdlog::info("[Balance Server]: Remote {0} Request To Remove Chatting GRPC Server",
                    request->cur_server());
          ...
          if (status) {
                    spdlog::info("[Balance Server]: Remote [{0}] Removed From Chatting GRPC Server Successful!",
                              request->cur_server());
          }
          else {
                    spdlog::warn("[Balance Server]: Remote [{0}] Removed From Chatting GRPC Server Failed!",
                              request->cur_server());
          }
...
grpc::GrpcChattingImpl::GetInstancePeers(::grpc::ServerContext *context,
          ...
          if (details::GrpcDataLayer::get_instance()->findItemFromServer(request->cur_server(),
                    ...
                    spdlog::warn(
                              ...
                              "Retrieve Data From Chatting Server Instance Failed! Because of Nothing Found!",
                              request->cur_server());
          }
          return grpc::Status::OK;
}
