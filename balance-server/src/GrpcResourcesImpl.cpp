...
::grpc::Status grpc::GrpcResourcesImpl::RegisterInstance(
    ...
    ::message::StatusResponse *response) {
          ...
          spdlog::info("[Balance Server]: Remote {0} Request To Register Resources Instance Server",
                    request->info().name());

          //Already Exist
          if (details::GrpcDataLayer::get_instance()->findItemFromServer(request->info().name(),
                    details::SERVER_TYPE::RESOURCES_SERVER_INSTANCE)) {

                    /*server has already exists*/
                    spdlog::warn("[Balance Server]: Remote {0} Try To Register Resources Instance Failed"
                              " Because of Resources Instance Server Has Already Exists!",
                              request->info().host());

                    response->set_error(
                              static_cast<std::size_t>(ServiceStatus::RESOURCES_SERVER_ALREADY_EXISTS));

                    return grpc::Status::OK;
          }

          /*Because We have already know it is exist or not!*/
          auto& lists = details::GrpcDataLayer::get_instance()->getResourcesServerInstances();

          auto resources_peer = std::make_unique<grpc::details::ServerInstanceConf>(
                    request->info().host(), request->info().port(),
                    request->info().name());

          lists.insert(std::make_pair(request->info().name(), std::move(resources_peer)));

          spdlog::info("[Balance Server]: Remote  [{0}-{1}:{2}] Registered To Resources Instances Server Successful!",
...
grpc::GrpcResourcesImpl::RegisterGrpc(::grpc::ServerContext *context,
          ...
          spdlog::info("[Balance Server]: Remote {0} Request To Register Resources GRPC Server",
                    request->info().name());
          ...
          if (details::GrpcDataLayer::get_instance()->findItemFromServer(request->info().name(),
                    ...
                    /*server has already exists*/
                    spdlog::warn("[Balance Server]: Remote {0} Try To Register Resources GRPC Server Failed"
          ...
          /*Because We have already know it is exist or not!*/
          auto& lists = details::GrpcDataLayer::get_instance()->getResourcesGRPCServer();
          ...
          spdlog::info("[Balance Server]: Remote  [{0}-{1}:{2}] Registered To Resources GRPC Server Successful!",
                    request->info().name(), request->info().host(),
                    request->info().port());
...
::grpc::Status grpc::GrpcResourcesImpl::ShutdownInstance(
    ...
    ::message::StatusResponse *response) {
          ...
          spdlog::info("[Balance Server]: Remote {0} Request To Remove Resources Instance Server",
                    request->cur_server());

          bool status = details::GrpcDataLayer::get_instance()->removeItemFromServer(request->cur_server(),
                    details::SERVER_TYPE::RESOURCES_SERVER_INSTANCE);

          if (status) {
                    spdlog::info("[Balance Server]: Remote [{0}] Removed From Resources Instance Server Successful!",
                              request->cur_server());
          }
          else {
                    spdlog::warn("[Balance Server]: Remote [{0}] Removed From Resources Instance Server Failed!",
                              request->cur_server());
          }
...
grpc::GrpcResourcesImpl::ShutdownGrpc(::grpc::ServerContext *context,
          ...
          spdlog::info("[Balance Server]: Remote {0} Request To Remove Resources GRPC Server",
                    request->cur_server());
          ...
          if (status) {
                    spdlog::info("[Balance Server]: Remote [{0}] Removed From Resources GRPC Server Successful!",
                              request->cur_server());
          }
          else {
                    spdlog::warn("[Balance Server]: Remote [{0}] Removed From Resources GRPC Server Failed!",
                              request->cur_server());
          }
...
grpc::GrpcResourcesImpl::GetInstancePeers(::grpc::ServerContext *context,
          ...
          if (details::GrpcDataLayer::get_instance()->findItemFromServer(request->cur_server(),
                    details::SERVER_TYPE::RESOURCES_SERVER_INSTANCE)) {
                    ...
                    spdlog::warn(
                              ...
                              "Retrieve Data From Resources Server Instance Failed! Because of Nothing Found!",
                              request->cur_server());
          }
