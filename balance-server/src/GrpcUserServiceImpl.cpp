...
::grpc::Status 
grpc::GrpcUserServiceImpl::RegisterUser(::grpc::ServerContext* context,
                                                                      const ::message::UserRegisterRequest* request,
                                                                      ::message::UserRegisterResponse* response) {

          /*get the lowest load server*/
          auto server_opt = details::GrpcDataLayer::get_instance()
                    ->serverInstanceLoadBalancer(details::SERVER_TYPE::CHATTING_SERVER);
          ...
          std::optional<std::string> exists_opt = details::GrpcDataLayer::get_instance()->getUserToken(request->uuid());
          ...
          details::GrpcDataLayer::get_instance()->registerUserInfo(request->uuid(), token);
...
grpc::GrpcUserServiceImpl::LoginUser(::grpc::ServerContext* context,
          ...
          response->set_error(static_cast<std::size_t>(
                    details::GrpcDataLayer::get_instance()->verifyUserToken(request->uuid(), request->token())));
          return grpc::Status::OK;
}
