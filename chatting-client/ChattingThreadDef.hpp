...
struct FriendingConfirmInfo {
  ...
  FriendingConfirmInfo(const MsgType type, const QString &threadId,
                       const QString &messageId, const QString &sender,
                       const QString &receiver, const QString &content)

      : message_type(type), thread_id(threadId), message_id(messageId),
...
struct ChattingRecordBase {
  ...
  ChattingRecordBase(const QString &sender, const QString &receiver,
                     MsgType _type = MsgType::DEFAULT)

      : sender_uuid(sender), receiver_uuid(receiver), type(_type),
        m_uniqueId(QUuid::createUuid().toString()) {}

  // User input unique_id!
  ChattingRecordBase(const QString &sender, const QString &receiver,
                     const QString &unique_id, MsgType _type = MsgType::DEFAULT)
...
struct ChattingTextMsg : public ChattingRecordBase {
  ...
  const QString &unsafe_getMsgID() const override { return m_data->m_msg_id; }
...
struct ChattingVoice : public ChattingRecordBase {
  ...
  const QString &getMsgContent() override { return m_msg_content; }
  void setMsgID(const QString &value) override {
    m_msg_id = value;
    this->setVerified(true);
  }
  const QString &unsafe_getMsgID() const override { return m_msg_id; }
...
static constexpr bool allowed_types =
    std::is_same_v<ChattingRecordBase, std::decay_t<_Ty>> ||
...
struct ChatMsgPageResult {
  ...
  ChatMsgPageResult(const bool load_more, const QString &thread_id,
                    const QString &next_msg_id,
                    std::vector<std::shared_ptr<ChattingRecordBase>> &&lists)
      : m_load_more(load_more), m_thread_id(thread_id),
        m_next_message_id(next_msg_id), m_list(std::move(lists)) {}
...
public:
  ...
  explicit UserChatThread(const QString &thread_id, const UserNameCard &card,
      ...
      : m_threadId(thread_id), m_peerCard(std::make_shared<UserNameCard>(card)),
        m_userChatType(type), m_recordStorge() {}
...
public:
  ...
  [[nodiscard]]
  std::vector<std::shared_ptr<ChatType>> dumpAll() const {
  ...
  generatePackage(const FriendingConfirmInfo &info) {
    ...
    if (info.message_type == MsgType::TEXT) {
      res = std::make_shared<ChattingTextMsg>(
          info.message_sender, info.message_receiver, info.message_content);
  ...
  static std::shared_ptr<ChatType> generatePackage(const MsgType type,
    ...
    if (type == MsgType::TEXT) {
      res = std::make_shared<ChattingTextMsg>(sender, receiver, unique_id,
