...
void FileTCPNetwork::send_download_request(
  ...
  FileTCPNetwork::get_instance()->send_buffer(
      info->curr_sequence == 1 ? ServiceType::SERVICE_INITFILEFETCHINGREQUEST
  ...
  m_callbacks.insert(std::pair<ServiceType, Callbackfunction>(
      ...
      [this, uploadcallback](QJsonObject &&json) {
        ...
        [[maybe_unused]] auto current_block_size =
            json["current_block_size"].toString().toUInt();
        [[maybe_unused]] auto transfered_size =
            json["transfered_size"].toString().toUInt();
        [[maybe_unused]] auto total_size =
            json["total_size"].toString().toUInt();

        [[maybe_unused]] auto eof = json["EOF"].toBool();

        emit signal_breakpoint_download(
            std::make_shared<FileTransferDesc>(
                filename, checksum, filepath, curr_seq, last_seq, eof,
                transfered_size, total_size, TransferDirection::Download),

            /*base64 => normal*/
            QByteArray::fromBase64(data_block.toUtf8()), current_block_size);
      }));
}
