...
std::optional<QPixmap> ImageCropperQLabel::getCropppedImage(const CroppingShape shape){
    /*
    ...
    */
    std::size_t start_x_pos = static_cast<std::size_t>((m_croppedRect.left() - m_imageRect.left()) / m_ratio);
    std::size_t start_y_pos = static_cast<std::size_t>((m_croppedRect.top() - m_imageRect.top()) / m_ratio);

    std::size_t cropped_width = static_cast<std::size_t>(m_croppedRect.width() / m_ratio);
    std::size_t cropped_height = static_cast<std::size_t>(m_croppedRect.height() / m_ratio);

    //ret value
    QPixmap res(cropped_width, cropped_height);
    //copy image from the original image
    res = m_originalImage.copy(start_x_pos, start_y_pos, cropped_width, cropped_height);
...
void ImageCropperQLabel::mouseMoveEvent(QMouseEvent *ev){
    ...
    if(!isLbuttonPressed
        || !m_imageRect.contains(m_currMovedPos)){
        return;
    }
...
void ImageCropperQLabel::fillRectBlank(QPoint central, std::size_t length, QColor color){
    QRect rect(central.x() - length * 0.5f,central.y() - length * 0.5f, length, length);
    QPainter painter(this);
    painter.fillRect(rect, color);
}

const bool ImageCropperQLabel::isMouseInsideSelectionSquare(const QPoint &p1, const QPoint &p2) const{

    return std::abs(p1.x() - p2.x()) * 2 <= m_selectionSquareLength &&
           std::abs(p1.y() - p2.y()) * 2 <= m_selectionSquareLength;
}

const CroppingPosition ImageCropperQLabel::getPositionInCropperRect(const QPoint &p){

    if(isMouseInsideSelectionSquare(p, QPoint(m_croppedRect.right(), m_croppedRect.center().y()))){
        return CroppingPosition::RIGHT_MID;
    }
    else if(isMouseInsideSelectionSquare(p, QPoint(m_croppedRect.left(),m_croppedRect.center().y()))){
        return CroppingPosition::LEFT_MID;
    }
    else if(isMouseInsideSelectionSquare(p, QPoint(m_croppedRect.center().x(),m_croppedRect.bottom()))){
        return CroppingPosition::BUTTOM_MID;
    }
    else if(isMouseInsideSelectionSquare(p, QPoint(m_croppedRect.center().x(),m_croppedRect.top()))){
...
void ImageCropperQLabel::resetCropper(){
    switch(m_shape){
        ...
        case CroppingShape::ELLIPSE:
            ...
            m_croppedRect.setRect(
                static_cast<int>(label_width * 0.5 - length * 0.5f),
