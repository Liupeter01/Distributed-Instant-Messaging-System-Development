...
private:
  ...
  void fillRectBlank(QPoint central, std::size_t length,
                     QColor color = Qt::white);
  [[nodiscard]] const bool isMouseInsideSelectionSquare(const QPoint &p1,
                                                        const QPoint &p2) const;
  [[nodiscard]] const CroppingPosition
  getPositionInCropperRect(const QPoint &p);
  void moveSelectionFrame(const CroppingPosition &pos, int x_offset,
                          int y_offset);
