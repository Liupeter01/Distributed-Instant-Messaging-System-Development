...
[[nodiscard]]
std::size_t
LogicExecutor::calculateBlockNumber(const std::size_t totalSize,
...
void LogicExecutor::registerCallbacks() {
    ...
    m_callbacks.insert(std::pair<ServiceType, Callbackfunction>(
        ServiceType::SERVICE_FILECHECKUPLOADPROGRESSRESPONSE, updateNextBlocksInfo));
}
...
void LogicExecutor::slot_send_next_block(const QString& checksum){
    ...
    FileTCPNetwork::get_instance()->send_buffer(ServiceType::SERVICE_FILEUPLOADREQUEST,
                                           std::move(obj));

    file.close();
}

void LogicExecutor::slot_start_file_transmission(const QString &fileName, const QString &filePath, const std::size_t fileChunk)
{
    m_fileName = fileName;
    m_filePath = filePath;
   ...
   */
    ...
    LogicMethod::get_instance()->recordMD5Progress(/* QString */m_fileCheckSum,
...
void LogicExecutor::slot_resume_file_transmission(){
    ...
    FileTCPNetwork::get_instance()->send_buffer(ServiceType::SERVICE_FILECHECKUPLOADPROGRESSREQUEST,
                                           std::move(obj));
}
