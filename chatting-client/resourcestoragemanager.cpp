...
void ResourceStorageManager::recordUnfinishedTask(
    const QString &checksum, std::shared_ptr<FileTransferDesc> info) {
...
bool ResourceStorageManager::recordQLabelUpdateLists(const QString &path,
  ...
  // create process
  auto [_, status] = m_batch_qlabels.try_emplace(
      path, std::vector<std::shared_ptr<QLabel>>{s_ptr});
  return status;
}
...
bool ResourceStorageManager::executeQLabelUpdateLists(const QString &path) {
  ...
  for (auto ib = it->second.begin(); ib != it->second.end(); ib++) {
    ...
    QPixmap pixmapScaled(pixmap.scaled((*ib)->size(), Qt::KeepAspectRatio,
