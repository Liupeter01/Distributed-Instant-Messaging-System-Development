...
TCPNetworkBase::TCPNetworkBase(const MsgNodeType _type,
                               const TargetServer _server,
                               QObject *parent)
    : QObject{}, m_type(_type),m_server(_server),
...
TCPNetworkBase::~TCPNetworkBase(){
    emit signal_terminate_server();
}
...
void TCPNetworkBase::setupDataRetrieveEvent(QTcpSocket &socket, RecvInfo &received, RecvNodeType &buffer)
{
