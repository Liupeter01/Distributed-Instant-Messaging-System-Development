...
void UserAccountManager::addItem2List(
  ...
  auto [friend2thread_iterator, friendMapThreadStatus] = m_friendOnThreadsLists.try_emplace(friend_uuid, thread_id);
  if(!friendMapThreadStatus){
      qDebug() << "friend uuid already exist! remove this chat thread in early stage\n";
...
void UserAccountManager::addItem2List(std::shared_ptr<UserChatThread> info) {
    ...
    if (!info || !m_userInfo || !info->getUserNameCard()) return;
  ...
  // create mapping relation of uuid<->thread_id
  // and relation between thread_id <-> ChattingThreadDesc
  auto thread_desc = (type == UserChatType::GROUP)
  ...
  // because all process should be controlled by one lock!
    auto [auth_iterator, authFriendStatus] = m_auth_friend_list.try_emplace(friend_uuid, namecard);
    if(!authFriendStatus){
    ...
    // relation between thread_id <-> ChattingThreadDesc
    auto [desc_iterator, threadDescStatus] = m_threadDescLists.try_emplace(thread_id, thread_desc);
    if(!threadDescStatus){
         //rollback
        qDebug() << "add ChattingThreadDesc to Map already exist!\n";
    ...
    auto [thread2Chatthread_iterator, threadid2ChatThreadStatus] =  m_ThreadData.try_emplace(thread_id, info);
