...
void SyncLogic::handlingUserChatTheads(ServiceType srv_type,
  ...
  auto list_status = mysql->get()->getUserChattingThreadIdx(
      std::stoi(uuid), std::stoi(thread_id),
      /*interval*/ 10, next_thread_id, is_complete);
...
void SyncLogic::handlingCreateNewPrivateChat(ServiceType srv_type,
  ...
  auto friend_uuid =
      boost::json::value_to<std::string>(src_root["friend_uuid"]);
  ...
  auto status = mysql->get()->createNewPrivateChat(std::stoi(my_uuid),
                                                   std::stoi(friend_uuid));

  if (!status.has_value()) {
    spdlog::warn("[{}]: Failed To Generate Private Chat Thread ID Between "
                 "UUID: [{}]<->UUID:[{}]!",
                 ServerConfig::get_instance()->GrpcServerName, my_uuid,
                 friend_uuid);
...
void SyncLogic::handlingFriendRequestConfirm(ServiceType srv_type,
  ...
  auto generate_json =
      [](const std::string &thread_id, const std::string &friend_uuid,
  ...
  auto src_uuid = boost::json::value_to<std::string>(
      src_root["src_uuid"]); // target user's uuid
  auto dst_uuid = boost::json::value_to<std::string>(
      src_root["dst_uuid"]); // confimer's uuid
  auto alternative =
  ...
  auto status = mysql->get()->execFriendConfirmationTransaction(
      std::stoi(src_uuid), std::stoi(dst_uuid));
  ...
  auto json =
      generate_json(thread_id, src_uuid, message_arr, std::move(*src_info));
  ...
  if (server_op.value() == ServerConfig::get_instance()->GrpcServerName) {
    ...
    if (!session_op.has_value()) {
      generateErrorMessage("Invalid Session",
                           ServiceType::SERVICE_FRIENDING_ON_BIDDIRECTIONAL,
                           ServiceStatus::FRIENDING_ERROR, *session_op);
      return;
    }
    ...
    boost::json::object root =
        generate_json(thread_id, dst_uuid, message_arr, std::move(*dst_info));
    /*propagate the message to dst user*/
    (*session_op)
...
void SyncLogic::handlingTextChatMsg(ServiceType srv_type,
 ...
 std::vector<std::shared_ptr<chat::MsgInfo>> updated_msg;
  ...
  if (!(src_root.contains("text_sender") &&
        src_root.contains("text_receiver") && src_root.contains("text_msg") &&
  ...
  // RAII
  {
    ...
    for (auto &item : arr) {
      ...
      updated_msg.push_back(std::make_shared<chat::TextMsgInfo>(
          thread_id, boost::json::value_to<std::string>(obj["unique_id"]),
  ...
  if (!server_op) {
    generateErrorMessage(
        fmt::format("[{}] Cannot Find Receiver {}'s Server Info In  Redis",
                    ServerConfig::get_instance()->GrpcServerName,
                    receiver_uuid),
        ServiceType::SERVICE_TEXTCHATMSGRESPONSE,
  ...
  for (auto &item : updated_msg) {
    ...
    if (!item->isVerified) {
      spdlog::error(
          "[{} ]: UUID = {} Send Request To UUID = {} Error, Unexpected Error!",
