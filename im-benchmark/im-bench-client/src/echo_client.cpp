...
void BenchSession::do_read() {
          auto self = shared_from_this();
          sock_.async_read_some(buffer(rbuf_),
                    [this, self](auto ec, auto n) {
                              if (ec) { stats_.failed++; return; }
                              if (n >= HEADER_SIZE + sizeof(uint64_t)) {
                                        ...
                                        if (hdr.id == BENCH_ECHO_ID && hdr.body_len() >= sizeof(uint64_t)) {
                                                  ...
                                                  uint64_t t_ms = duration_cast<milliseconds>(
                                                            clk::now() - stats_.start).count();
                                                  {
