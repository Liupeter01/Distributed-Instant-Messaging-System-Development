...
int main(int argc, char** argv) {
          ...
          std::vector<io_context> iocs(nt);
          std::vector<std::thread> threads;
          std::vector<executor_work_guard<io_context::executor_type>> guards;
          ...
          for (int i = 0; i < nt; i++) {
                    guards.emplace_back(make_work_guard(iocs[i]));
          ...
          for (auto& s : stats.samples) rtts.push_back(s.rtt_us);
          ...
          std::cout << "wrote " << stats.samples.size()
                    << " samples to " << csv << "\n";
}