...
public:
  virtual ~RequestHandlerDispatcher() { shutdown(); }
  void commit(pair &&recv_node, [[maybe_unused]] SessionPtr live_extend) {
    ...
    if (!temp.has_value()) {
      spdlog::error("[{}]: Dispatcher Error While Handling Session: {}",
                    ServerConfig::get_instance()->GrpcServerName, session_id);
      return;
    }
...
private:
  ...
  RequestHandlerDispatcher(std::size_t threads) : m_convertor() {
    for (std::size_t thread = 0; thread < threads; ++thread) {
      m_nodes.emplace_back(
          std::make_shared<handler::RequestHandlerNode>(thread));
    }
  }
  void shutdown() {
  ...
  dispatch_to_node(std::string_view session_id) {
    ...
    try {
      ...
      spdlog::error("[{}]: Retrieve Request Handler Node Error, Reason:{}",
