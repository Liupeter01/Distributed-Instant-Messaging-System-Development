...
class RequestHandlerDispatcher : public Singleton<RequestHandlerDispatcher> {
  ...
  using ContainerType = tbb::concurrent_vector<std::shared_ptr<handler::RequestHandlerNode>>;
...
private:
          ...
          std::optional<std::shared_ptr<handler::RequestHandlerNode>> dispatch_to_node(std::string_view session_id);
