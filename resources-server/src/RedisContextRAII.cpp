...
bool redis::RedisContext::setValue(const std::string &key,
  ...
  if (status) {
    spdlog::info(
        "[Redis]: Execute command [ SET key = {0}, value = {1}] successfully!",
        key.c_str(), value.c_str());
    return true;
  }
  return false;
...
bool redis::RedisContext::setValue2Hash(const std::string &key,
  ...
  if (status) {
    spdlog::info(
        "[Redis]: Execute command [ HSET key = {0}, field = {1}, value = {2}] "
        "successfully!",
        key.c_str(), field.c_str(), value.c_str());
    return true;
  }
...
bool redis::RedisContext::leftPush(const std::string &key,
  ...
  auto status = m_replyDelegate->redisCommand(*this, std::string("LPUSH %s %s"),
                                              key.c_str(), value.c_str());
  if (status) {
    spdlog::info("[Redis]: Execute command  [ LPUSH key = {0}, value = {1}]  "
                 "successfully!",
                 key.c_str(), value.c_str());
    return true;
  }
...
bool redis::RedisContext::rightPush(const std::string &key,
  ...
  auto status = m_replyDelegate->redisCommand(*this, std::string("RPUSH %s %s"),
                                              key.c_str(), value.c_str());
  if (status) {
    spdlog::info("[Redis]: Execute command  [ RPUSH key = {0}, value = {1}]  "
                 "successfully!",
                 key.c_str(), value.c_str());
    return true;
  }
...
bool redis::RedisContext::delPair(const std::string &key) {
  ...
  if (status) {
    spdlog::info("[Redis]: Execute command [ DEL key = {} ]successfully!",
                 key.c_str());
    return true;
  }
  return false;
...
redis::RedisContext::checkValue(const std::string &key) {
  ...
  spdlog::info("[Redis]: Execute command [ GET key = %s ] successfully!",
               key.c_str());
...
redis::RedisContext::leftPop(const std::string &key) {
  ...
  spdlog::info("[Redis]: Execute command [ LPOP key = {} ] successfully!",
               key.c_str());
  return m_replyDelegate->getMessage();
}
...
redis::RedisContext::rightPop(const std::string &key) {
  ...
  spdlog::info("[Redis]: Execute command  [ RPOP key = {}] successfully!",
               key.c_str());
  return m_replyDelegate->getMessage();
}
...
bool redis::RedisContext::setValueExp(const std::string &key,
                                      const std::string &field,
                                      const std::size_t EXPX, TimeUnit unit) {
  ...
  auto status = m_replyDelegate->redisCommand(*this, inputSchema, key.c_str(),
                                              EXPX, field.c_str());

  if (status) {
    spdlog::info(
        "Execute command [ SETEX/PX key = {0}, expire = {1}, value = {2}] "
        "successfully!",
