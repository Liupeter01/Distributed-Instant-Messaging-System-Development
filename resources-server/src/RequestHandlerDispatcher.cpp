...
dispatcher::RequestHandlerDispatcher::RequestHandlerDispatcher(std::size_t threads) : m_convertor() {
          for (std::size_t thread = 0; thread < threads; ++thread) {
                    m_nodes.emplace_back(std::make_shared<handler::RequestHandlerNode>(thread));
          }
}

dispatcher::RequestHandlerDispatcher::~RequestHandlerDispatcher() { shutdown(); }
...
void dispatcher::RequestHandlerDispatcher::commit(pair&& recv_node,
          ...
          if (auto opt = dispatch_to_node(session_id); opt) {
                    ...
                    spdlog::info("[Resources Server]: Dispatcher Network Request To Node {} Successfully!", (*opt)->getId());
                    return;
          }

          spdlog::error("[Resources Server]: Dispatcher Error While Handling Session: {}", session_id);
}

std::size_t dispatcher::RequestHandlerDispatcher::hash_to_index(std::string_view session_id) const {
          return m_convertor(session_id) % m_nodes.size();
}

dispatcher::RequestHandlerDispatcher::ContainerType::iterator
dispatcher::RequestHandlerDispatcher::dispatch_to_iterator(std::string_view session_id) {
          if (m_nodes.empty()) {
...
dispatcher::RequestHandlerDispatcher::dispatch_to_node(std::string_view session_id) {
